...
public class ControlaJogador3 : MonoBehaviourPunCallbacks
{
...
public class BETA_SETTINGS{
        ...
        if (fsmode == FSMode.Dynamic)
        {
            ...
            if (Physics.Raycast(transform.position, Vector3.down, out hit))
            {
                ...
                if (IsGrounded)
                {
                    if (!previousGrounded)
                    {
                        if (dynamicFootstep.currentClipSet.Any()) { audioSource.PlayOneShot(dynamicFootstep.currentClipSet[UnityEngine.Random.Range(0, dynamicFootstep.currentClipSet.Count)], Volume / 10); }
    ...
    public IEnumerator CameraShake(float Duration, float Magnitude)
    {
        ...
        {
            playerCamera.transform.localPosition = Vector3.MoveTowards(playerCamera.transform.localPosition, new Vector3(cameraStartingPosition.x + UnityEngine.Random.Range(-1, 1) * Magnitude, cameraStartingPosition.y + UnityEngine.Random.Range(-1, 1) * Magnitude, cameraStartingPosition.z), Magnitude * 2);
...
public class FPAIO_Editor2 : Editor
{
    FirstPersonAIO t;
